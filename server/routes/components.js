...
// GET all components
router.get('/', (req, res) => {
    const query = 'SELECT * FROM asset_components ORDER BY created_at DESC';
...
router.get('/:id', (req, res) => {
    const componentQuery = 'SELECT * FROM asset_components WHERE id = ?';
...
router.put('/:id', upload.single('image'), (req, res) => {
    ...
    db.query('SELECT image_url FROM asset_components WHERE id = ?', [req.params.id], (err, data) => {
        if (err) return res.status(500).json(err);
...
router.delete('/:id', (req, res) => {
    ...
    db.query('SELECT image_url FROM asset_components WHERE id = ?', [req.params.id], (err, data) => {
        ...
        db.query('DELETE FROM asset_components WHERE id = ?', [req.params.id], (err, data) => {
            if (err) return res.status(500).json(err);
...
router.post('/:id/checkout', (req, res) => {
    ...
    db.query('SELECT available_quantity FROM asset_components WHERE id = ?', [componentId], (err, data) => {
        if (err) return res.status(500).json(err);
        ...
        // Create assignment
        const assignQuery = 'INSERT INTO asset_component_assignments (component_id, assigned_to, quantity, notes) VALUES (?, ?, ?, ?)';
        db.query(assignQuery, [componentId, assigned_to, quantity, notes], (err, data) => {
            if (err) return res.status(500).json(err);

            // Update available quantity
            const updateQuery = 'UPDATE asset_components SET available_quantity = available_quantity - ? WHERE id = ?';
...
router.post('/:id/checkin', (req, res) => {
    ...
    db.query('SELECT * FROM asset_component_assignments WHERE id = ?', [assignment_id], (err, data) => {
        if (err) return res.status(500).json(err);
        ...
        // Delete assignment
        db.query('DELETE FROM asset_component_assignments WHERE id = ?', [assignment_id], (err, data) => {
            if (err) return res.status(500).json(err);
