...
const ComponentList = () => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axios.delete(`http://localhost:5000/api/components/${componentToDelete}`);
            setComponents(components.filter((comp) => comp.id !== componentToDelete));
            alert('Component deleted successfully!');
        } catch (error) {
            console.error('Error deleting component:', error);
            alert(error.response?.data?.message || 'Error deleting component. Please try again.');
        } finally {
